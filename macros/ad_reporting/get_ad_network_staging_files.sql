{% macro get_ad_network_staging_files() %}
    ...
    {% if 'pinterest_ads' in var("attribution_warehouse_ad_sources") %}
    {% set _ = staging_file.append(ref('int_pinterest_ads')) %}
    {% endif %}

    {% if 'microsoft_ads' in var("attribution_warehouse_ad_sources") %}
    {% set _ = staging_file.append(ref('int_microsoft_ads')) %}
    {% endif %}

    {% if 'linkedin_ads' in var("attribution_warehouse_ad_sources") %}
    {% set _ = staging_file.append(ref('int_linkedin_ads')) %}
    {% endif %}

    {% if 'twitter_ads' in var("attribution_warehouse_ad_sources") %}
